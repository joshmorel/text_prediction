...
def print_history_words(history_words):
    # to get json formatting as desired
    for (score, word) in history_words.words:
        print("{}\t{}\t{}".format(history_words.history, word, score))
...
def main(args):
    ...
    # opening of model table
    print("history\tword\tscore")
            ...
            if order in order_history:
                ...
                if history == order_history[order].history:
                    ...
                    if order_history[order].words:
                        print_history_words(order_history[order])
                    if word not in excluded_tokens and count >= parser.freq_limit:
    ...
    print_history_words(order_history[1])
